...
@Api(value = "employee", produces = MediaType.APPLICATION_JSON) 
public class EmployeeResourceController {
	...
	@Consumes({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	public Response updateEmployee(@PathParam("id") Long id, Employee employee) throws URISyntaxException {
	...
	@Consumes({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	public Response getEmployeeById(@PathParam("id") Long id) throws URISyntaxException {
		boolean isEmployeeExist = employeeService.isEmployeeExist(id);
		if (isEmployeeExist) {
			Employee employee = employeeService.getEmployee(id);
			return Response.status(200).entity(employee).contentLocation(new URI("/employee/" + id)).build();
		} else {
			return Response.status(404).entity("Employee not found").build();
		}
	}
	...
	@Consumes({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	public Response deleteEmployee(@PathParam("id") Long id) throws URISyntaxException {
		boolean isEmployeeExist = employeeService.isEmployeeExist(id);
		if (isEmployeeExist) {
